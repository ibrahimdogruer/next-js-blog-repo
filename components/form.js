      ...
      <div className="mt-4">
        {isAuthenticated ? (
          <div className="flex items-center space-x-2">
            ...
            <img src={user.picture} width={30} className="rounded-full" onError={(e)=>{e.target.onerror = null; e.target.src="/avatar.jpeg"}} />
            <span>{user.name}</span>
            <button
              typeof="button"
              onClick={() =>
                logout({
                  returnTo: process.env.NEXT_PUBLIC_URL + "/blog?redirectUrl=" + url,
                })
              }
            >
